...
//! `PRELOOP_GITHUB_TOKEN` and then the local HMAC JWT. A *failed* mint is
...
///
/// Two naming pairs are accepted: `PRELOOP_GITHUB_APP_PEM{,_FILE}` and the older
/// `PRELOOP_GITHUB_APP_PRIVATE_KEY{,_PATH}` already published in
/// `docs/github-app-webhook.md`.
const PRIVATE_KEY_ENV: [(&str, bool); 4] = [
    ("PRELOOP_GITHUB_APP_PEM", false),
    ("PRELOOP_GITHUB_APP_PEM_FILE", true),
    ("PRELOOP_GITHUB_APP_PRIVATE_KEY", false),
    ("PRELOOP_GITHUB_APP_PRIVATE_KEY_PATH", true),
];
...
const MINT_FAILURE_ENV: &str = "PRELOOP_GITHUB_APP_MINT_FAILURE";

/// What a job's `GITHUB_TOKEN` becomes when installation-token minting fails.
///
/// `PRELOOP_GITHUB_TOKEN` is a static PAT: it ignores `permissions:`, is not
...
pub(crate) enum MintFailurePolicy {
    ...
    Error,
    /// Fall back to `PRELOOP_GITHUB_TOKEN`, accepting its broader authority.
    Pat,
}

impl MintFailurePolicy {
    /// Read the policy from `PRELOOP_GITHUB_APP_MINT_FAILURE`, falling back to
...
pub(crate) struct GitHubAppCredentials {
    ...
    /// Comes from `PRELOOP_GITHUB_TOKEN` or the config file's `github.pat`.
...
/// [`preloop_gha_parser::DEFAULT_TOKEN_PERMISSIONS`] — this function does not
...
fn installation_id_override() -> anyhow::Result<Option<u64>> {
    let Some(raw) = env_non_empty("PRELOOP_GITHUB_APP_INSTALLATION_ID") else {
        return Ok(None);
    };
    raw.parse()
        .map(Some)
        .with_context(|| format!("PRELOOP_GITHUB_APP_INSTALLATION_ID={raw} is not a number"))
}
...
mod tests {
    ...
    async fn mints_a_scoped_token_per_job_and_caches_installation_discovery() {
        ...
        let defaulted = mint_for_repository(
            ...
            &creds,
            "Preloop/other-repo",
            &preloop_gha_parser::effective_token_permissions(None),
            false,
        )
        .await
