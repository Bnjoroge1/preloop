...
fn build_expression_context_has_required_roots() {
    ...
    let val = preloop_gha_expressions::eval_expression("github.repository", &expr_ctx);
    assert!(val.is_ok());

    // Verify steps context resolves
    let steps_val = preloop_gha_expressions::eval_expression("steps.step1.conclusion", &expr_ctx);
    assert!(steps_val.is_ok());

    // Verify success() evaluates correctly
    let success = preloop_gha_expressions::eval_bool("success()", &expr_ctx);
...
fn job_status_failure_reflects_in_context() {
    ...
    let expr_ctx = ctx.build_expression_context();
    let success = preloop_gha_expressions::eval_bool("success()", &expr_ctx).unwrap();
    assert!(!success);
    let failure = preloop_gha_expressions::eval_bool("failure()", &expr_ctx).unwrap();
    assert!(failure);
}
...
fn vars_context_decodes_typed_dict_format() {
    ...
    // {"t": 2, "d": [{"k": "PRELOOP_REPO_ROOT", "v": {"t": 1, "d": "/workspace"}}]}
    let ctx = JobContext::new(
        "j1".into(),
        "Test".into(),
        serde_json::json!({}),
        serde_json::json!({
            ...
            "vars": {
                "t": 2,
                "d": [
                    {"k": "PRELOOP_REPO_ROOT", "v": {"t": 1, "d": "/workspace"}},
    ...
    let val = preloop_gha_expressions::eval_expression("vars.PRELOOP_REPO_ROOT", &expr);
    assert_eq!(val.unwrap().as_str(), Some("/workspace"));
    let val2 = preloop_gha_expressions::eval_expression("vars.OTHER_VAR", &expr);
...
fn cancelled_status_reflects_in_context() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert!(!preloop_gha_expressions::eval_bool("success()", &expr_ctx).unwrap());
    assert!(!preloop_gha_expressions::eval_bool("failure()", &expr_ctx).unwrap());
    assert!(preloop_gha_expressions::eval_bool("cancelled()", &expr_ctx).unwrap());
...
fn matrix_context_resolves_in_expressions() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert_eq!(
        preloop_gha_expressions::eval_expression("matrix.os", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("ubuntu-latest")
    );
    assert_eq!(
        preloop_gha_expressions::eval_expression("matrix.node", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("20")
    );
...
fn needs_context_resolves_in_expressions() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert_eq!(
        preloop_gha_expressions::eval_expression("needs.build.result", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("success")
    );
    assert_eq!(
        preloop_gha_expressions::eval_expression("needs.build.outputs.sha", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("abc123")
    );
...
fn strategy_context_resolves_in_expressions() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert!(preloop_gha_expressions::eval_bool("strategy.fail-fast", &expr_ctx).unwrap());
}
...
fn env_context_resolves_in_expressions() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert_eq!(
        preloop_gha_expressions::eval_expression("env.MY_VAR", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("hello")
    );
    assert_eq!(
        preloop_gha_expressions::eval_expression("env.OTHER", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("world")
    );
...
fn secrets_context_resolves_in_expressions() {
    ...
    let expr_ctx = ctx.build_expression_context();
    assert_eq!(
        preloop_gha_expressions::eval_expression("secrets.MY_SECRET", &expr_ctx)
            .unwrap()
            .as_str(),
        Some("s3cr3t")
    );
