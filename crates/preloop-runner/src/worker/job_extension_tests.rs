...
fn setup_workspace_clears_stale_repository() {
    ...
    let unique = format!("__preloop_test_work_{}", std::process::id());
...
#[test]
fn build_step_list_parses_preloop_template_string_maps() {
    let steps = vec![serde_json::json!({
        "id": "step1",
        "reference": {
            "type": "script"
...
"#,
    ...
    assert!(matches!(
        ...
        StepType::Action { with, .. }
            if with.get("__preloop_entry").and_then(|v| v.as_str()) == Some("pre.js")
    ));
    assert!(matches!(
        &ordered[2].step_type,
        StepType::Action { with, .. }
            if with.get("__preloop_entry").and_then(|v| v.as_str()) == Some("cleanup.js")
    ));
...
fn lifecycle_local_actions_skip_pre_but_retain_main_and_post() {
    ...
    assert!(matches!(
        &ordered[4].step_type,
        StepType::Action { with, .. }
            if with.get("__preloop_entry").and_then(|v| v.as_str()) == Some("post.js")
    ));
}
...
"#,
    ...
    assert!(matches!(
        &ordered[0].step_type,
        StepType::Action { with, .. }
            if with.get("__preloop_entry").and_then(|v| v.as_str()) == Some("pre-entrypoint.sh")
    ));
    assert!(matches!(
        &ordered[2].step_type,
        StepType::Action { with, .. }
            if with.get("__preloop_entry").and_then(|v| v.as_str()) == Some("post-entrypoint.sh")
    ));
    assert_eq!(ordered[2].condition.as_deref(), Some("always()"));
}
