...
pub struct AgentJobRequestMessage {
    ...
    #[serde(
        rename = "preloopDebugRunId",
        default,
        skip_serializing_if = "Option::is_none"
    )]
    pub preloop_debug_run_id: Option<String>,

    /// aksh extension: transport mode for DAP traffic.
    #[serde(
        rename = "preloopDebugTransport",
        default,
        skip_serializing_if = "Option::is_none"
    )]
    pub preloop_debug_transport: Option<String>,
    ...
    ///
    #[serde(
        rename = "preloopSnapshotOriginRewrite",
        default,
        skip_serializing_if = "Option::is_none"
    )]
...
pub(crate) fn template_string_token(value: &str) -> serde_json::Value {
    ...
    loop {
        let Some(start) = rest.find("${{") else {
            append_format_literal(&mut literal, rest);
            break;
        };
        append_format_literal(&mut literal, &rest[..start]);
...
/// Append a literal format-string segment, escaped: `'` -> `''`, `{` -> `{{`,
/// `}` -> `}}`.
///
/// MC-S6: the official runner's `format()` throws `InvalidFormatString` on an
/// unescaped lone `{`/`}`, so any template mixing a literal brace with an
/// expression breaks on dispatch unless it is escaped here. This is the single
/// implementation — `preloop-gha-parser`'s job builder calls it too, so the two
/// emitters cannot drift apart again.
pub fn append_format_literal(target: &mut String, literal: &str) {
...
mod tests {
    ...
    fn job_message_debug_redacts_snapshot_auth_header() {
        ...
        assert_eq!(
            wire["preloopSnapshotOriginRewrite"]["authHeader"],
